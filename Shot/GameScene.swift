...
class GameScene: SKScene {
    ...
    override  func touchesMoved(touches: NSSet, withEvent event: UIEvent) {
        ...
        if (preLocation != nil) {
            ...
            } else if _player!.position.x > (CGRectGetMaxX(self.frame) - _playerFrame.width){
                _player!.position.x = CGRectGetMaxX(self.frame) - _playerFrame.width
            }
            
            _player!.position.y += _location.y - preLocation!.y
            if _player!.position.y < 0 {
                _player!.position.y = 0
            } else if _player!.position.y > (CGRectGetMaxY(self.frame) - _playerFrame.height){
                _player!.position.y = CGRectGetMaxY(self.frame) - _playerFrame.height
            }
